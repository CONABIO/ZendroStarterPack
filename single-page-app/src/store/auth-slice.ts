...
export const logUserIn = createAsyncThunk<
  ...
  UserRequest,
  { state: AuthState }
>('auth/logUserIn', async ({ username, password }: UserRequest, thunkAPI) => {
  const { user, error } = await authenticateFromRemote(username, password);
  if (error) throw error;
  const state = thunkAPI.getState();
  if (state.status === 'cancelled')
    throw new AuthError(
      AUTH_REQUEST_CANCELLED,
      `Cancelled user login: ${username}`
    );
  return user;
});
