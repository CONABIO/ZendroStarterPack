...
const definition = {
    ...
    associations: {
        ...
        unique_cumulus_criteria: {
            type: 'one_to_one',
            implementation: 'foreignkeys',
            reverseAssociation: 'unique_cumulus',
...
module.exports = class cumulus extends Sequelize.Model {
    ...
    /**
     * add_devices_ids - field Mutation (model-layer) for to_many associationsArguments to add
     *
     * @param {Id}   id   IdAttribute of the root model to be updated
     * @param {Array}   devices_ids Array foreign Key (stored in "Me") of the Association to be updated.
     */
    static async add_devices_ids(id, devices_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            devices_ids.forEach(idx => {
                promises.push(models.device_catalog.add_cumulus_ids(idx, [`${id}`], benignErrorReporter, false));
            });
        ...
        if (record !== null) {
            let updated_ids = helper.unionIds(JSON.parse(record.devices_ids), devices_ids);
            updated_ids = JSON.stringify(updated_ids);
            await record.update({
                devices_ids: updated_ids
            });
        }
    }
    /**
     * add_institutions_ids - field Mutation (model-layer) for to_many associationsArguments to add
     *
     * @param {Id}   id   IdAttribute of the root model to be updated
     * @param {Array}   institutions_ids Array foreign Key (stored in "Me") of the Association to be updated.
     */
    static async add_institutions_ids(id, institutions_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            institutions_ids.forEach(idx => {
                promises.push(models.institution.add_cumulus_ids(idx, [`${id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
        ...
        let record = await super.findByPk(id);
        if (record !== null) {
            let updated_ids = helper.unionIds(JSON.parse(record.institutions_ids), institutions_ids);
            updated_ids = JSON.stringify(updated_ids);
            await record.update({
                institutions_ids: updated_ids
            });
        }
    }
    ...
    /**
     * remove_devices_ids - field Mutation (model-layer) for to_many associationsArguments to remove
     *
     * @param {Id}   id   IdAttribute of the root model to be updated
     * @param {Array}   devices_ids Array foreign Key (stored in "Me") of the Association to be updated.
     */
    static async remove_devices_ids(id, devices_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            devices_ids.forEach(idx => {
                promises.push(models.device_catalog.remove_cumulus_ids(idx, [`${id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
        ...
        let record = await super.findByPk(id);
        if (record !== null) {
            let updated_ids = helper.differenceIds(JSON.parse(record.devices_ids), devices_ids);
            updated_ids = JSON.stringify(updated_ids);
            await record.update({
                devices_ids: updated_ids
            });
    ...
    /**
     * remove_institutions_ids - field Mutation (model-layer) for to_many associationsArguments to remove
     *
     * @param {Id}   id   IdAttribute of the root model to be updated
     * @param {Array}   institutions_ids Array foreign Key (stored in "Me") of the Association to be updated.
     */
    static async remove_institutions_ids(id, institutions_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            institutions_ids.forEach(idx => {
                promises.push(models.institution.remove_cumulus_ids(idx, [`${id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
        ...
        let record = await super.findByPk(id);
        if (record !== null) {
            let updated_ids = helper.differenceIds(JSON.parse(record.institutions_ids), institutions_ids);
            updated_ids = JSON.stringify(updated_ids);
            await record.update({
                institutions_ids: updated_ids
            });
