...
async function validForDeletion(id, context) {
    if (await countAllAssociatedRecords(id, context) > 0) {
        throw new Error(`transect with id ${id} has associated records and is NOT valid for deletion. Please clean up before you delete.`);
    }
    return true;
}
...
module.exports = {
    ...
    transects: async function({
        ...
        if (await checkAuthorization(context, 'transect', 'read') === true) {
            ...
            return await transect.readAll(search, order, pagination, benignErrorReporter);
        } else {
    ...
    transectsConnection: async function({
        ...
        if (await checkAuthorization(context, 'transect', 'read') === true) {
            ...
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return await transect.readAllCursor(search, order, pagination, benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    readOneTransect: async function({
        ...
        if (await checkAuthorization(context, 'transect', 'read') === true) {
            ...
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return await transect.readById(id, benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    countTransects: async function({
        ...
        if (await checkAuthorization(context, 'transect', 'read') === true) {
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return await transect.countRecords(search, benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    /**
     * vueTableTransect - Return table of records as needed for displaying a vuejs table
     *
     * @param  {string} _       First parameter is not used
     * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
     * @return {object}         Records with format as needed for displaying a vuejs table
     */
    vueTableTransect: async function(_, context) {
        if (await checkAuthorization(context, 'transect', 'read') === true) {
            return helper.vueTable(context.request, transect, ["id"]);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    addTransect: async function(input, context) {
        ...
        if (authorization === true) {
            ...
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            let createdTransect = await transect.addOne(inputSanitized, benignErrorReporter);
            await createdTransect.handleAssociations(inputSanitized, benignErrorReporter);
    ...
    /**
     * bulkAddTransectCsv - Load csv file of records
     *
     * @param  {string} _       First parameter is not used
     * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
     */
    bulkAddTransectCsv: async function(_, context) {
        if (await checkAuthorization(context, 'transect', 'create') === true) {
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return transect.bulkAddCsv(context, benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    deleteTransect: async function({
        ...
        if (await checkAuthorization(context, 'transect', 'delete') === true) {
            if (await validForDeletion(id, context)) {
                let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
                return transect.deleteOne(id, benignErrorReporter);
            }
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    ...
    updateTransect: async function(input, context) {
        ...
        if (authorization === true) {
            ...
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            let updatedTransect = await transect.updateOne(inputSanitized, benignErrorReporter);
            await updatedTransect.handleAssociations(inputSanitized, benignErrorReporter);
    ...
    bulkAssociateTransectWithNode_id: async function(bulkAssociationInput, context) {
        ...
        return await transect.bulkAssociateTransectWithNode_id(bulkAssociationInput.bulkAssociationInput, benignErrorReporter);
    },
    /**
    ...
    bulkDisAssociateTransectWithNode_id: async function(bulkAssociationInput, context) {
        ...
        return await transect.bulkDisAssociateTransectWithNode_id(bulkAssociationInput.bulkAssociationInput, benignErrorReporter);
    },
    ...
    csvTableTemplateTransect: async function(_, context) {
        if (await checkAuthorization(context, 'transect', 'read') === true) {
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return transect.csvTableTemplate(benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    }
